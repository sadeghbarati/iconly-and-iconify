...
async function bootstrap(weight: string) {
  const iconlyIconset = await importDirectory(`${getWeightDir('iconly', weight)}`, {
    prefix: 'iconly',
  ...
  await exportJSONPackage(iconlyIconset, {
    target: weight === '*' ? path.join(process.cwd(), '../iconly-unofficial-json-all') : path.join(process.cwd(), `../iconly-unofficial-json-${weight}`),
    package: {
      name: `@iconly-unofficial-json/${weight === '*' ? 'all' : weight}`,
      version: '0.0.1',
      iconSetVersion: '2.3',
  ...
  await exportIconPackage(iconlyIconset, {
    target: weight === '*' ? path.join(process.cwd(), '../iconly-unofficial-icons-all') : path.join(process.cwd(), `../iconly-unofficial-icons-${weight}`),
    module: true,
    package: {
      name: `@iconly-unofficial-icons/${weight === '*' ? 'all' : weight}`,
      version: '0.0.1',
      bugs: 'https://github.com/iconly-unofficial/iconly/issues',
      homepage: 'https://github.com/iconly-unofficial/iconly',
    },
